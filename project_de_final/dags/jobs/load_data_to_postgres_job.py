...
def load_data_to_postgres():
    ...
    data_files = get_data_files(s3_connection, bucket_name)
    for index in range(0, len(data_files), 100):
        dataframe = get_dataframe_from_data_files(spark, data_files[index:index+100])
        dataframe = transform_dataframe(dataframe)
        dataframe.show()
        save_dataframe_to_db(db_connection, dataframe)
